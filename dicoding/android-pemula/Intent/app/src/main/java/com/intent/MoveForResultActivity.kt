    ...
    override fun onClick(v: View?) {
        if (v?.id == R.id.btn_choose) {
            if (rgNumber.checkedRadioButtonId > 0) {
                ...
                // data for the result will be append to the new empty intent
                // then that intent will be set to the set result with parameter code and intent
                // then fun finish to send back result
